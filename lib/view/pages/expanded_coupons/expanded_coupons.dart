...
import 'package:spense_app/util/helper/size_helper.dart';
import 'package:spense_app/view/pages/expanded_coupons/components/coupon_page.dart';
import 'package:spense_app/view/widgets/circular_border_hero.dart';
...
class _ExpandedCouponsState extends State<ExpandedCoupons> {
  ...
  void onScroll() {
    setState(() {
      ...
      if (couponsController.hasClients)
        couponsController
            .jumpTo(currentPage * MediaQuery.of(context).size.width);
      if (imagesController.hasClients)
        imagesController.jumpTo(
            currentPage * MediaQuery.of(context).size.width * imageFactor);
    });
  }
  ...
  void initState() {
    // PageController
    pageController = PageController(initialPage: widget.initialCoupon);
    ...
    couponsController = ScrollController(
        initialScrollOffset:
            currentPage * MediaQuery.of(widget.buildContext).size.width);

    // Images SrollController
    imagesController = ScrollController(
        initialScrollOffset: currentPage *
            (MediaQuery.of(widget.buildContext).size.width * imageFactor));
                              ...
                              children: [
                                ...
                                Container(
                                  height: MediaQuery.of(context).size.height *
                                      cCouponTopSizeFactor,
                                  decoration: BoxDecoration(
                                      borderRadius: BorderRadius.only(
                                          bottomLeft:
                                              Radius.circular(cCouponPadding),
                                          bottomRight:
                                              Radius.circular(cCouponPadding)),
                                      color: Colors.white),
                                  child: Padding(
                                    padding: const EdgeInsets.only(
                                        right: cCouponPadding, bottom: 20.0),
                                    child: Align(
                                ...
                                SingleChildScrollView(
                                  ...
                                  child: Row(
                                    children: List.generate(couponsList.length,
                                        (index) {
                                      return SizedBox(
                                        width:
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Row(
                        ...
                        final double rotation =
                            (currentPage - index).clamp(-1.0, 1.0);

                        return Transform.translate(
                          offset: Offset(getWidth(64.0), getWidth(156.0)),
                          child: Transform.rotate(
                            angle: couponsList[index].animationType ==
